...
```{python}
import os
# Required for AMD/ROCm GPU detection on some Linux systems; harmless on other platforms.
...
def train_mlp(hidden_sizes, X_train, X_dev, X_test, y_train, y_dev, y_test,
    ...
    extras = {"snapshots": snapshots} if do_snapshots else {}
...
The training loop above always runs for exactly `N_EPOCHS`. In practice, dev loss often starts rising before training loss does — the model has started memorising rather than generalising. **Early stopping** halts training when the dev loss stops improving and keeps the weights from the best epoch instead.
...
| Concept | Key takeaway | Section |
|---------|-------------|---------|
| MLP | Stacks linear layers + ReLU activations; `nn.Sequential` chains them | §1.1 |
| Training loop | `zero_grad` → `backward` → `step` each mini-batch | §1.2 |
| train/eval mode | `model.train()` enables dropout/batchnorm training behaviour; `model.eval()` disables it | §1.2, §4 |
| Capacity | Width/depth control what the network can represent; too much → overfitting | §2 |
| Train/dev/test split | Train subset used for gradient updates; dev set used to monitor training and tune hyperparameters; test set touched only for final evaluation | §1.2 |
| Learning curves | Train/dev gap is the primary signal for over- and underfitting | §2 |
| Learning rate | Too small: slow convergence. Too large: instability. Adam adapts per-parameter | §3 |
| Weight decay | L2 penalty on weights; reduces overfitting by keeping weights small | §4 |
| Dropout | Random activation masking; forces robust, distributed representations | §4 |
| Batch norm | Normalises layer inputs; stabilises training and acts as regulariser | §4 |
| Early stopping | Halt at best dev-loss epoch; restore saved weights — free regularisation | §4 |
